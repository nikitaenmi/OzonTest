...
## Описание проекта
- Задача сделать мок-сервис, при этом обеспечить надёжность и воспроизводимость тестов.

- Сложность задачи обусловлена требованием к нагрузке в сотни запускаемых параллельно тестов. А также, по условию, нельзя пробрасывать параметры в мок для управления.

- Поэтому для решения данной задачи было принято решение сделать API с POST ручкой на gRPC-gatewate с http like интерфейсом. 

- Пользователь отправляет запрос - API идёт в мок-сервис - получает ответ с курсом валюты - API возращает данные пользователю.

- Пунктом выше, видно, что на сервис приходит POST запрос, с датой, количеством, валютой и суммой. 
Результат работы сервиса - возрат отправленной суммы в рублях.

 - В рамках сжатых сроков было принято решение не писать логику завязанную на провайдерах - просто сохраняем в базу о том что якобы на такой-то площадке были осуществлены торги. Помимо этого в этой БД сохраняется полная информация о платеже.
