...
## Описание проекта
Задача сделать мок-сервис, при этом обеспечить надёжность и воспроизводимость тестов. Для решения данной задачи было принято решение сделать помимо мок-сервиса, API, позволяющие обрабатывать множество паралелльных запросов.
...
Результат работы сервиса - возрат отправленной суммы в рублях.
 ...
 В рамках сжатых сроков было принято решение не писать логику завязанную на провайдерах - просто сохраняем в базу о том что якобы на такой-то площадке были осуществлены торги. Помимо этого в этой БД сохраняются полная информация о платеже.
