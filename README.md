...
## Описание проекта
- Задача сделать сервис, для тестирования оплаты. Тесты должны быть воспроизводимы и надёжны, поэтому сервис должен выдавать воспроизводимые результаты.
...
- С frontend(а) приходит запрос на оплату, поэтому в проекте также реализовано API с POST ручкой. В этой же API происходит основная валидация платежа - по условию задачи платёж должен быть менее 15000 рублей.

- API берёт курс валюты по данным с frontend(а) и проверяет 

- Если всё успешно API делает запись в БД о платеже, и возращает сумму в рублях.

- пользователь отправляет запрос ---> API идёт в мок-сервис ---> получает ответ с курсом валюты ---> API возращает данные пользователю

- Пунктом выше, видно, как конкретно общается сервис с пользователем.
