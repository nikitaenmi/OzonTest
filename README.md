...
- С frontend(а) приходит запрос на оплату, поэтому в проекте реализовано API с REST-like ручкой сгенерированной gRPC gateway. В этом же API происходит основная валидация платежа - по условию задачи платёж должен быть менее 15000 рублей

### Рабочий flow сервиса
- API берёт данные о валюте и сумме с frontend(а) и вызывает мок-сервис ЦБ РФ

- Мок-сервис возращает ответ в формате XML курс валют к рублю, API парсит ответ и валидирует сумму (должно быть <15000 руб. и т.д.)
...
- В рамках сжатых сроков было принято решение не писать логику завязанную на провайдерах, а сохрянть в БД информацию о площадке, на которой был совершен платёж.

- Для проверки роботоспособоности приложения был реализан нагрузочный тест.

- API с frontend(а) принимает данные о дате, провайдере, валюте (USD, EUR и т.д.), сумме, а возращает то же самое, только сумму в рублях и ID платежа. А мок-сервис возращает только XML.
